...
re(function () {
    if (typeof re('#avatar-upload-form') !== undefined) {
        initAvatarUpload();
        re('#avatar-max-size').html(maxSizeAllowed);
        re('#avatar-upload-form input:file').on("change", function (e) {
            var files = e.currentTarget.files;
            for (var x in files) {
                if (files[x].name != "item" && typeof files[x].name != "undefined") {
                    if (files[x].size <= maxSizeInBytes) {
                        // Submit the selected file
                        re('#avatar-upload-form .upload-file-notice').removeClass('bg-danger');
                        re('#avatar-upload-form').submit();
                    } else {
                        // File too large
                        re('#avatar-upload-form .upload-file-notice').addClass('bg-danger');
                    }
                }
            }
        });
...
function initAvatarUpload() {
    re('#avatar-upload-form').ajaxForm({
        beforeSend: function () {
            updateProgress(0);
            re('#avatar-upload-form').addClass('hidden');
        },
        ...
        success: function (data) {
            ...
            if (data.success === false) {
                re('#status').html(data.errorMessage);
            } else {
                re('#preview-pane .preview-container img').attr('src', data.fileName);
                var img = re('#crop-avatar-target');
                img.attr('src', data.fileName);

                if (!keepUploadBox) {
                    re('#avatar-upload-box').addClass('hidden');
                }
                re('#avatar-crop-box').removeClass('hidden');
                initAvatarCrop(img);
            }
        },
...
function updateProgress(percentComplete) {
    re('.upload-percent-bar').width(percentComplete + '%');
    re('.upload-percent-value').html(percentComplete + '%');
    if (percentComplete === 0) {
        re('#upload-status').empty();
        re('.upload-progress').removeClass('hidden');
    }
}
...
function initAvatarCrop(img) {
    img.Jcrop({
        ...
        var pcnt = re('#preview-pane .preview-container');
        xsize = pcnt.width();
        ysize = pcnt.height();
        re('#preview-pane').appendTo(jcrop_api.ui.holder);
        jcrop_api.focus();
    });
}
...
function updatePreviewPane(c) {
    if (parseInt(c.w) > 0) {
        ...
        re('#preview-pane .preview-container img').css({
...
function saveAvatar() {
    var img = re('#preview-pane .preview-container img');
    re('#avatar-crop-box button').addClass('disabled');
    ...
    re.ajax({
        ...
        if (data.success === true) {
            re('#avatar-result img').attr('src', data.avatarFileLocation);

            re('#avatar-result').removeClass('hidden');

            if (!keepCropBox) {
                re('#avatar-crop-box').addClass('hidden');
            }
        } else {
