    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                user.AvatarUrl = "http://res.cloudinary.com/lifedemotivator/image/upload/default.jpg";
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                    var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                    await UserManager.SendEmailAsync(user.Id, "E-mail confirmation", "Подтвердите вашу учетную запись, щелкнув" + "<a href=\"" + callbackUrl + "\">здесь</a>");
